...
class ClTrack(object):
    ...
    def on_slot_fired(self):
        self.__parent.send_message("fired on track : " + str(self.trackNum) + " slot #" + str(self.track.fired_slot_index))
        if self.clipSlot != -1 and (self.clipSlot.is_triggered or self.clipSlot.is_playing or self.clipSlot.is_recording or self.clipStopping == True):
    ...
    def onClipChange(self):
        ...
        if self.clipSlot != -1 and self.clipSlot.has_clip and self.clipSlot.clip.playing_status_has_listener(self.onClipStatusChange):
